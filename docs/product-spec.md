...
| SP-203 | Human decision and permission request | Agent raises a structured, deduplicated decision or scoped permission request in the ticket; the Product Owner sees the exact action and rationale, chooses **Allow** for the current run or **Deny**, and the same turn continues without a terminal or global binary whitelist |
