...
### Candidate review state table

| Durable state | Entering command or event | SQLite evidence | Owner sees | Available actions | Relaunch recovery |
| --- | --- | --- | --- | --- | --- |
| Immutable candidate awaiting review | Implementation completion records the candidate before any review starts; repository-changing candidates first enter the serialized integration queue, while repository-free outcomes can proceed directly. | `candidate_revisions` binds the implementation run, delivery kind, base SHA, candidate head SHA, result JSON, and ticket workspace; the delivery note and proposed knowledge changes are candidate-scoped. | The Ticket is **In review** with an inspectable candidate and handoff. | Pause or stop the sprint; await independent review. | The scheduler reads the candidate row and never reconstructs a candidate from a completed callback. |
| Independent review running | The review workflow prepares a clean read-only workspace at the exact candidate or integrated SHA and creates a Tech Lead run. | `candidate_revisions.status = reviewing` with `integration_worktree_path` and, after integration, `integrated_sha`; a distinct Tech Lead `agent_runs` row stores its thread and workspace. | **In review**, Tech Lead activity, and attributed work-log updates. | Pause or stop the sprint; answer an exact scoped permission request if one is raised. | A queued/running/interrupted Tech Lead run resumes only when its workspace still matches the candidate. A missing thread is replaced against the same immutable revision. |
| Review approved before integration | The Tech Lead approves a repository-changing candidate that has not yet been integrated. | The review run is `completed`; delivery-note verification and knowledge proposals are `reviewed`; `candidate_revisions.status = queued_for_integration`. | The Ticket stays **In review** while integration waits or runs. | Await integration; pause or stop the sprint. | The integration queue uses the reviewed candidate ID and exact head SHA. Review is not repeated after a later clean re-integration of the same approved candidate. |
| Integrated review approved | The Tech Lead approves the exact integrated revision, or approves a repository-free outcome. | The review run is `completed`; candidate-scoped knowledge is reviewed; the candidate retains its exact integrated SHA or local-outcome base; subsequent demo/finalization evidence belongs to the next subdomain. | The Ticket advances toward **Ready for demo** without presenting raw review reasoning. | Use the later demo and acceptance actions. | Finalization derives from the approved candidate row and completed review run, not an in-memory result. |
| Changes requested | The Tech Lead returns concrete findings. | The review run is `completed`; `candidate_revisions.status = changes_requested`; candidate knowledge proposals are `superseded`; the Ticket returns to `running`; the same implementation run/workspace receives the review feedback. | **In progress** with attributed findings and revision activity. | Automatic revision continues up to the bounded correction limit; the owner may pause or stop. | The integrated result, when present, is adopted as the ticket-workspace baseline before the same implementation thread resumes. |
| Needs product owner direction | Material findings remain after the bounded automatic review returns. | The candidate remains `changes_requested`; the implementation run is `awaiting_owner`; the Tech Lead work-log entry lists the remaining findings. | Inline **Needs your input** with the findings and answer controls. | Ask the assigned team member a question or add direction and submit answers. | The exact implementation run resumes from its preserved workspace only after owner direction. |
| Review interrupted or failed | App shutdown/pause interrupts the Tech Lead turn, or review/post-review handoff fails. | Interruption keeps the candidate `reviewing` and marks the Tech Lead run `interrupted`; a non-cancellation failure marks the candidate/run `failed`, returns the Ticket to `running`, and places the implementation run `awaiting_owner` with a system work-log entry. | A recoverable review state or explicit retry message on the Ticket. | Resume the sprint or comment to retry from preserved evidence. | Recovery selects the latest review run bound to the candidate and rejects mismatched workspaces; completed reviews can resume an interrupted post-review handoff without attesting again. |

### Integration and conflict resolution state table

| Durable state | Entering command or event | SQLite evidence | Owner sees | Available actions | Relaunch recovery |
| --- | --- | --- | --- | --- | --- |
| Queued for serialized integration | A repository-changing candidate is produced or an approved demo revision becomes stale against current local `trunk` or verified GitHub state. | `candidate_revisions.status = queued_for_integration` retains the exact candidate head and any completed review; the Ticket is `integrating` or `verifying`. | **In review** with an inline integrating state; no separate board column. | Pause or stop the sprint; await the serialized merge. | The scheduler derives the queue from candidate rows and admits one candidate at a time against current `trunk`. |
| Integrating exact candidate | The integration queue claims the candidate and creates a clean integration worktree from current local `trunk`. | `candidate_revisions.status = integrating`; after a clean merge it records `integrated_sha` and `integration_worktree_path`. | **In review** with bounded integration activity. | Pause or stop the sprint. | A candidate with a durable integration snapshot resumes at review/finalization; an interrupted merge is retried from candidate and current-trunk evidence rather than an in-memory callback. |
| Conflict resolution running | Candidate merge or verified GitHub incorporation reports conflicted files and starts a focused Integrator run in the preserved integration worktree. | `candidate_revisions.status = resolving_conflict` with the worktree path; a distinct Tech Lead `agent_runs` row stores the conflict thread and workspace; the work log identifies the conflict source. | **In review**, Integrator activity, and an attributed explanation without raw Git commands. | Pause or stop; answer a focused conflict question when requested. | A ready-to-commit worktree completes directly. A queued run resumes the same worktree/thread; a missing thread is replaced without discarding conflict state. |
| Conflict needs owner input | The Integrator returns a structured question instead of resolving the overlap. | The resolution run is `awaiting_owner`; its work-log comment stores the prompt and options while the candidate remains `resolving_conflict`. | Inline **Needs your input** on the Ticket. | Submit an answer, pause, or stop. | The owner response queues the same resolution run; the scheduler resumes it from SQLite and the preserved worktree. |
| Integrated and awaiting exact review | A clean merge completes, including any verified GitHub changes. | The candidate records the final `integrated_sha`; incorporated remote SHA is recorded in the work log; candidate status advances to `reviewing` when the exact Tech Lead review starts. | **In review** with the exact integrated revision under review. | Pause or stop; await review. | Review receives the recorded integration worktree and SHA. A conflict-resolved or remotely changed result must be reviewed; a clean re-integration of an already approved unchanged candidate retains its prior review. |
| Integration failed or interrupted | Worktree creation, merge, remote verification, or conflict execution fails or the app stops. | Ordinary failure marks the candidate `failed`, returns the Ticket to `running`, and places the implementation run `awaiting_owner`; conflict failure keeps `resolving_conflict` and marks its run `awaiting_owner` or `interrupted`. Work-log evidence preserves the cause. | A recoverable owner-facing explanation and retry action. | Add direction and retry, resume the sprint, or stop. | Candidate, ticket workspace, integration worktree, and run/thread identities remain durable; retry never invents a replacement candidate. |

### Acceptance and finalization state table

| Durable state | Entering command or event | SQLite evidence | Owner sees | Available actions | Relaunch recovery |
| --- | --- | --- | --- | --- | --- |
| Preparing the reviewed result | The exact-candidate Tech Lead review approves a repository revision or repository-free outcome. | The implementation and review runs are `completed`; repository candidates retain `integrated_sha` and an integration worktree; candidate status advances toward `ready_for_demo`. | **In review** while Spedito validates the managed demo and connected pull-request contract. | Pause or stop the sprint; wait for preparation. | A durable integration snapshot resumes preparation without rerunning implementation or changing candidate identity. |
| Ready for product owner approval | Managed demo validation succeeds, or a reviewed repository-free outcome needs no demo. | `candidate_revisions.status = ready_for_demo`; the Ticket is `acceptance`; accepted/rejected knowledge proposals remain candidate-bound; any demo session and pull-request publication are durable. | **Ready for demo** with the reviewed result, work log, knowledge decisions, and **Approve and complete**. | Review the result, decide required knowledge changes, comment for correction, or approve and complete. | Relaunch reloads the same candidate, proposal decisions, demo session, and remote publication. |
| Completing approval | The product owner chooses **Approve and complete**. | `candidate_revisions.status = promoting`; the acceptance operation rechecks the exact local and GitHub heads before promotion, then publishes only eligible knowledge and follow-up suggestions. | The detail may close immediately; the board shows a bounded **Completing** state rather than optimistic Done. | Wait; if a remote head moved, review the newly integrated candidate instead. | `promoting` and already `accepted` candidates whose Ticket is not yet Done are re-admitted from SQLite. Each step is idempotent or checks existing durable evidence. |
| Candidate became stale | Current local `trunk` or the connected pull-request head no longer matches the reviewed candidate during completion. | The same candidate returns to `queued_for_integration`; the Ticket returns to integration/review with an attributed work-log explanation. | **In review**, not Done; the owner is told that the latest verified changes must be integrated and reviewed. | Wait for reintegration and exact-candidate review, or stop the sprint. | The normal integration queue resumes from the candidate row and preserved workspaces. |
| Accepted and completed | Exact promotion/reconciliation, knowledge publication, follow-up suggestion creation, and Ticket transitions all succeed. | The candidate is `accepted`; the Ticket transitions through `ready_to_release` to `released`; one completion work-log entry records the accepted revision or repository-free outcome; a finished sprint is persisted when applicable. | **Done** with the completion handoff and accepted product knowledge. | Inspect history, launch accepted app versions when applicable, or continue the sprint. | Relaunch derives Done, accepted history, product knowledge, and sprint completion from SQLite and accepted `trunk`. |
| Completion failed or was interrupted | Any post-acknowledgement validation, remote operation, promotion, publication, or transition fails, or the app stops. | A failed `promoting` candidate returns to `ready_for_demo`; interruption leaves `promoting` or `accepted` for recovery; a system work-log entry records the cause without duplicating completion. | The reviewed result remains available with a recoverable completion explanation; Done is not shown. | Choose **Approve and complete** again after resolving the cause, or relaunch to resume an interrupted operation. | Recovery re-admits the same candidate. Existing status, comments, proposal publications, remote merge state, and accepted SHA prevent duplicate promotion or publication. |

### Delivery recovery state table

| Durable state | Entering command or event | SQLite evidence | Owner sees | Available actions | Relaunch recovery |
| --- | --- | --- | --- | --- | --- |
| Sprint paused with active work | The product owner chooses **Pause sprint**, or shutdown suspends delivery without cancelling ticket intent. | The sprint is `paused`; active run rows, candidate rows, permission decisions, comments, thread IDs, and workspace paths remain durable while live turns and demo sessions stop. | The sprint is paused; each Ticket retains its last trusted state and work log. | Resume or stop the sprint; decide any preserved permission request. | Relaunch does not admit work while paused. Resume reuses queued/interrupted runs and their preserved workspaces rather than creating replacement delivery. |
| Sprint stopped | The product owner confirms destructive **Stop sprint**. | The sprint becomes `cancelled`; accepted candidates and Done Tickets remain unchanged, unaccepted candidates are superseded, active runs are interrupted, unfinished Tickets return to `ready`, and the work log remains. | The sprint is stopped; completed history remains and unfinished Tickets are **Ready to pick**. | Inspect audit history or plan another sprint. | Cancelled sprint rows are terminal and are not re-admitted. |
| Expired permission or live turn | The process ends while a run is active or awaiting a capability decision. | The run, latest request, saved decision, thread, and workspace remain. A resumable saved decision queues the same run; an undecided request remains owner-blocked; non-resumable review activity becomes interrupted. | Preserved **Needs your input** or a queued recovery explanation, never a repeated request presented as new. | Decide the request, resume the sprint, or add direction. | Recovery classifies the latest durable permission state once and conditionally updates the same run/request transactionally. |
| Orphaned implementation, review, or conflict run | Relaunch finds `running`, eligible `failed`, or interrupted delivery work without an owned live operation. | The existing `agent_runs` row retains the worktree and thread; the exact candidate keeps its delivery, integration, and review provenance. | The Ticket returns to its prior in-progress or review activity with a labelled recovery note when owner action is needed. | Resume automatically, answer a question, retry from the preserved run, pause, or stop. | A durable final response may complete the same run. Otherwise recovery queues the same implementer, exact-candidate review, or conflict run; it never invents a candidate. |
| Candidate queue or review snapshot interrupted | Relaunch finds `integrating`, `reviewing`, or `ready_for_demo` evidence that does not match the Ticket state. | Candidate status, exact SHAs, integration worktree, review run, and Ticket transitions are reconciled with compare-and-set recovery mutations. | The Ticket returns to **In review** or **Ready for demo** with no duplicate board transition. | Await recovery, pause, stop, or retry after an owner-visible failure. | Interrupted integration returns to the serialized queue; review restores only an exact matching workspace/SHA; invalid delivery evidence returns the preserved implementation to correction. |
| Acceptance interrupted after acknowledgement | Relaunch finds a `promoting` or `accepted` candidate whose Ticket is still `acceptance` or `ready_to_release`. | Candidate and Ticket rows, remote publication, accepted SHA, knowledge decisions, and existing completion comments remain authoritative. | **Completing** resumes; Done remains hidden until reconciliation finishes. | Wait or retry after a recoverable failure. | The acceptance task registry re-admits the same work item once; idempotent promotion/publication checks prevent duplicate completion. |
| Recovery precondition changed concurrently | A stale recovery snapshot loses a compare-and-set race or its workspace/SHA no longer matches. | The transaction rolls back all attempted candidate, run, request, Ticket, and comment mutations. | Current valid state remains; an error appears only in the owning product. | Retry from the newly loaded durable state. | The next scheduler/recovery pass reloads SQLite and derives a new decision instead of applying stale in-memory mutations. |
