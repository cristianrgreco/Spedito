...
| D10 | `TicketDeliveryWorkflowCoordinatorTests.d10SequentialDemoContractErrorsRepairToStaticPrototype`, `ProductScopedPersistenceTests.implementationRetryPreservesRunIdentity`, `TicketDeliveryRuntimeCoordinatorTests.staleCompletionCannotClearReplacement` | **Named/composed:** two sequential malformed Demo envelopes receive bounded focused corrections and resolve to the existing static prototype without discarding its workspace; a terminal failed or interrupted implementation requeues with preserved run identity, workspace, and thread while stale completion remains harmless. |
...
| D14 | `TicketDeliveryWorkflowCoordinatorTests.d14DemoRetryReusesReviewedCandidate`, `MacOSDemoLauncherTests.d14StaticWebPrototypeUsesHostOwnedServer`, `PriorityZeroShellJourneyUITests.testD14DemoRetrySelectsTheReviewedCandidate` | **Named:** the deterministic journeys retry host preparation only for the reviewed candidate and serve, reopen, and stop an exact static web prototype without a product runtime; the launched-process contract proves exact Demo selection. |
...
| D10 | P0 | Correct up to two sequential structured-result or Demo-contract failures against the preserved workspace before failing; then recover a failed/interrupted implementation through **Retry work**, reusing the durable workspace/thread contract without duplicating a completed transition. | J | — |
...
| D14 | P0 | Launch, reopen, stop, or retry the exact reviewed demo, including a host-served static web prototype; a host preparation failure retries preparation only, while a candidate failure requires a new candidate. | J | Y |
