...
| V05 | `DemoLaunchTests.acceptedAppVersionHistory`, `.acceptedApplicationPresentations`, `.unifiedAppVersionHistory`, `.invalidAcceptedCandidates` | **Composed:** independent tests order verified imported and accepted runnable demos newest first, include valid static web prototypes, and exclude incomplete, artifact, and command-output Candidate results; a new wrapper would duplicate those policy assertions. |
...
| V05 | P1 | List independently verified imported and accepted runnable demos, including static web prototypes, newest first; omit artifacts and command-output results. | J+P | — |
| V06 | P0 | Open, revisit, switch, stop, and retry exact demos; opening another version stops the active one and reconstructs the selected revision. | J | Y |
