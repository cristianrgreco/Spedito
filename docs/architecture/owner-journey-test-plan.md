...
| A04 | `ProductExecutionLifecycleTests.productSelectionDoesNotSuspendDelivery`, `TicketAttentionTests.reloadAggregatesBackgroundProductAttention`, `.acceptanceApprovalRefreshesSelectedProductAttention` | **Composed:** Product-scoped continuation and bounded cross-Product attention are independently deterministic; entering and leaving acceptance keeps the selected Product's attention fresh without a Product switch, including fresh-instance recovery. |
...
| D04 | `TicketDeliveryWorkflowCoordinatorTests.pausedDeliveryResumesExistingRun`, `.recoveryDoesNotRequeueItsOwnLiveRun`, `SQLiteStoreTests.sprintPauseAndResumeAreDurable` | **Composed:** a paused durable run relaunches and resumes its preserved workspace instead of restarting, and recovery leaves a run the running process still owns alone. |
...
| D08 | `TicketDeliveryWorkflowCoordinatorTests.d08PermissionReviewActionsPersistDistinctDecisions`, `CodexAdapterTests.scopedNetworkApprovalSaysItIsRestricted`, `PriorityZeroShellJourneyUITests.testD08PermissionReviewPresentsDenyAllowOnceAndAlwaysAllow` | **Named:** the deterministic journey persists Deny, Allow once, and Always allow as distinct decisions; the prompt words a scoped network request the same way the saved grant does; the launched-process contract proves all three review controls. |
...
| D10 | `TicketDeliveryWorkflowCoordinatorTests.d10SequentialDemoContractErrorsRepairToStaticPrototype`, `.d10PinnedDemoRecipeSurvivesKindFlippingRevision`, `.contestedDemoKindAsksTheOwnerAndAppliesTheAnswer`, `DemoLaunchTests.feedbackDemoChangeDetection`, `.revisionFeedbackPinsPriorRecipe`, `.continuationPinsChangesRequestedRecipe`, `.contractedSchemaAdmitsOnlyTheContractedBranch`, `.codeOnlyContractForcesANullDemo`, `.contestedKindOptionsRoundTrip`, `.perKindDecodeShapes`, `CodexAdapterTests.promptsCarryPinnedDemoRecipe`, `.proposedDemoKindDecoding`, `.promptsCarryTheDemoKindContract`, `WorkflowPolicyTests.deliveryDemoPolicyHonoursTheStoredContract`, `CodexLifecycleGuidanceTests.deliveryGuidanceCarriesEveryDemoShape`, `ProductScopedPersistenceTests.implementationRetryPreservesRunIdentity`, `TicketDeliveryRuntimeCoordinatorTests.staleCompletionCannotClearReplacement` | **Named/composed:** two sequential malformed Demo envelopes receive bounded focused corrections and resolve to the existing static prototype without discarding its workspace; a ticket's owner-approved demo kind — including `terminal_application`, whose contest option reads "opens in Terminal" and whose recipe must name the built workspace-relative program — narrows the result schema to that kind's branch (a `none` contract to a null demo), a contested kind pauses with Spedito's canonical decision options, and the owner's exact answer updates the contract durably before the continuation turn; delivery and review guidance forbid wrapping the product in another surface to satisfy the contract; a revision whose feedback does not name the demo carries the prior candidate's validated recipe forward verbatim — live and after relaunch recovery — instead of re-deciding the kind; a terminal failed or interrupted implementation requeues with preserved run identity, workspace, and thread while stale completion remains harmless. |
| D11 | `TicketDeliveryWorkflowCoordinatorTests.approvedReviewRemainsCandidateBound`, `.cleanReintegrationRetainsCandidateApproval`, `GitWorkspaceManagerTests.candidateLifecycle` | **Composed:** independent review and its handoff remain bound to the immutable candidate; clean re-integration after accepted trunk advances retains that approval without spawning another review. |
| D12 | `GitWorkspaceManagerTests.candidateLifecycle`, `.conflictResolutionLifecycle`, `WorkflowPolicyTests.candidateIntegrationsPrecedeReview`, `TicketDeliveryWorkflowCoordinatorTests.repeatedRecoveryKeepsReviewRunDispatchable`, `.recoveryReplacesOrphanedQueuedReviewRun` | **Composed:** exact integration, conflict resolution, and changed-result review ordering are covered at their owning boundaries; repeated relaunch recovery keeps the interrupted tech lead review dispatchable and retires review runs orphaned by an earlier candidate refresh. |
...
| D14 | `TicketDeliveryWorkflowCoordinatorTests.d14DemoRetryReusesReviewedCandidate`, `MacOSDemoLauncherTests.d14StaticWebPrototypeUsesHostOwnedServer`, `.terminalOpenFailureIsAHostFailure`, `.terminalMissingExecutableIsACandidateFailure`, `PriorityZeroShellJourneyUITests.testD14DemoRetrySelectsTheReviewedCandidate` | **Named:** the deterministic journeys retry host preparation only for the reviewed candidate and serve, reopen, and stop an exact static web prototype without a product runtime; a terminal app whose Terminal window fails to open or whose program never records its pid retries preparation, while a program that preparation did not build requires a new candidate; the launched-process contract proves exact Demo selection. |
| D15 | `SprintTicketWorkLogHistoryTests.d15ReadyForDemoCommentPreservesCandidate`, `TicketDeliveryWorkflowCoordinatorTests.d15DemoFeedbackRevisionSettlesAsNextCandidate`, `SQLiteStoreTests.sentBackCandidateDoesNotSwallowResumedDelivery`, `PriorityZeroShellJourneyUITests.testD15ReadyForDemoCommentKeepsCandidateActionable` | **Named:** the deterministic journeys prove a ready-for-demo owner comment preserves the reviewed candidate, and that Request changes supersedes it, requeues the exact run, and lets the revision loop settle as the next candidate version instead of being discarded as already settled — including when a send-back path leaves the run's settlement identity stale; the launched-process contract leaves the commented candidate actionable after the comment settles. |
...
| D18 | `TicketDeliveryWorkflowCoordinatorTests.failedAcceptanceRetriesWithoutDuplicateCompletion`, `SQLiteStoreTests.completedDeliverySettlementIsAtomicAndIdempotent` | **Composed:** the reviewed result survives failure and retries without duplicate promotion, while implementation completion persists one candidate and its work log, delivery note, retrospective evidence, run status, and ticket transition atomically. |
...
| D24 | `MacOSDemoLauncherTests.terminalLaunchWritesScriptAndOpensTerminal`, `.terminalScriptRunsTheProgramFromTheWorkspace`, `.terminalStopTerminatesTheProgram`, `.terminalReopenActivatesWhileAliveAndRelaunchesWhenDead`, `.terminalSmokeTestNeverRunsTheProgram`, `.terminalMissingExecutableIsACandidateFailure`, `.terminalOpenFailureIsAHostFailure`, `.terminalLaunchRejectsExecutableOutsideWorkspace`, `.realSandboxTerminalRecipeBuildsAndResolves`, `TerminalDemoLaunchScriptTests.exactScriptText`, `.currentDirectoryRunsFromTheWorkspaceRoot`, `.quoting`, `DemoLaunchTests.supportedPresentations`, `.perKindDecodeShapes`, `.acceptedApplicationPresentations`, `SQLiteStoreTests.demoKindMigrationV6DropsTheCheckWithoutLosingRows`, `KnowledgeContextSelectorTests.terminalCanonicalRecipeFollowsTheContract`, `SprintBoardDemoPresentationTests.readyExplanations`, `.stopDemo` | **Named:** preparation builds the program through the sandboxed executor and the smoke test resolves a regular executable file inside the checkout without running it; the launch writes the exact `TerminalDemoLaunchScript` with mode 0755 under the preview's runtime directory and opens it with Terminal, the executed script runs the program from the workspace with the isolated variables and records the program's pid, **Open demo** brings Terminal forward while the pid lives and relaunches once it has died, **Stop demo** ends the program within its bound and removes the pid file, a symlink or bare tool name never reaches Terminal, a missing program is a candidate failure while an open or pid failure is a host failure, a real-sandbox `swiftc` build resolves its program, migration v6 stores the kind without touching dependent rows, the canonical recipe page reaches only terminal-contracted runs, and the board explains the running program and offers Stop demo. |
...
| C09 | `TicketAttentionTests.c09VisibleTargetReadResolutionAndCounts`, `.visibleTicketTargetMarksExistingUpdatesRead`, `.runLeavingAwaitingOwnerResolvesTicketQuestion`, `.loadSweepRetiresEndedWaitsIdempotently`, `OwnerNotificationRecoveryPolicyTests`, `CodexTransportApplicationTests.refinementCompletionResolvesTicketQuestion` | **Named:** visible-target suppression, read, resolution, and deduplicated Product counts are composed in one deterministic journey; the sprint ticket sheet registers the same visibility, so reading a ticket anywhere counts; a needs-input row resolves when its wait ends by any route (run moves on, refinement completes without questions, plan lands), and the launch sweep retires ended waits idempotently while keeping owed decisions. |
| C10 | `TicketAttentionTests.notificationRouteRoundTrips`, `.archivedTargetDoesNotNavigate`, `.staleAcceptanceAttentionClickDropsNavigation` | **Composed:** every target kind round-trips, an archived target fails closed without navigation, and a stale attention click refreshes and drops instead of navigating to a released ticket. |
...
| V06 | `MacOSDemoLauncherTests.v06HistoricalAcceptedVersionLaunchesExactRevision`, `.terminalReopenActivatesWhileAliveAndRelaunchesWhenDead`, `.terminalStopTerminatesTheProgram`, `PriorityZeroShellJourneyUITests.testV06HistoricalAcceptedAppVersionIsIndependentlySelectable` | **Named:** the deterministic journey stops the active version and reconstructs the selected accepted revision; a terminal app version brings its Terminal window forward while the program lives, relaunches once it has died, and stops through the program's pid; the launched-process contract proves historical versions remain independently selectable. |
...
| E08 | `EpicPlanningJourneyTests.e08OwnerEpicEditsSurvivePlanningRetry` | **Named:** malformed plan output receives one repair attempt and then fails reviewably without changing the Epic or Backlog; owner edits to title, goal, success criteria, and constraints become authoritative retry input, survive the successful retry unchanged, and still create only reviewable proposals; the retry-needed question survives relaunch while the failure stands and resolves into a plan-ready update when the retried plan lands. |
| E09 | `EpicPlanningJourneyTests.e09EditedSuggestionAcceptsWithoutChangingUnrelatedProposals`, `EpicPlanningPresentationTests.e09SuggestionReviewDisclosesAcceptanceImpact`, `.proposalCardStatesTheReviewMedium`, `SQLiteStoreTests.demoKindContractsMigrateAndRoundTrip`, `CodexAdapterTests.suggestionDemoKindDecoding` | **Named:** the review projection discloses rationale, prerequisite scope, Backlog impact, the no-Sprint boundary, and the plain-language review medium the owner accepts with the plan — "Opens in Terminal" for a terminal program, planned by the mechanical four-surface rule the schema description repeats; editing every mutable proposal field preserves dependency identity through a fresh registry, acceptance creates the exact edited Ticket plus prerequisite carrying its approved demo kind, and unrelated proposals and Products remain unchanged. |
...
### 3.9 Priority 1 work packet 4 — Codebase and Demos
...
| V05 | `DemoLaunchTests.acceptedAppVersionHistory`, `.acceptedApplicationPresentations`, `.unifiedAppVersionHistory`, `.invalidAcceptedCandidates`, `RepositoryImportKnowledgeTests.terminalLaunchProposalImportsWhileArtifactsDoNot` | **Composed:** independent tests order verified imported and accepted runnable demos newest first, include valid static web prototypes and terminal apps, admit an imported terminal app proposal, and exclude incomplete, artifact, and command-output Candidate results; a new wrapper would duplicate those policy assertions. |
...
| Runnable versions available | Import an independently approved recipe or accept a runnable Candidate | Verified imported launch or accepted Candidate result and revision | One newest-first list containing only browser, static web prototype, macOS app, and terminal app versions | Select, open, stop, or retry a version | Fresh load rebuilds the same filtered order |
...
| V03 | `CodebaseDiffLayoutPreferenceTests.v03FileSelectionAndDiffLayoutResolution`, `.classifiesUnifiedDiffLines`, `DiffSyntaxHighlightingTests.v03ContentLinesSplitIntoMarkerAndRuns`, `.blockCommentSpansLines`, `.oldAndNewSidesDoNotBleed`, `.eachFileSectionUsesItsOwnLanguage`, `.headerlessHunkUsesTheReviewedPath`, `.markerlessAndTruncatedLinesStayPlain`, `.plainDocumentMatchesLineStructure`, `.diffHeaderPaths`, `.aliasedFileTypesResolveToTheirLanguage`, `.tokenColoursKeepContrastInBothAppearances` | **Named:** valid changed-file selection survives detail refresh, removed files clear safely, Auto selects unified below 900 points and side-by-side at or above it, explicit choices remain stable, and the persisted preference and unified line semantics are deterministic; syntax colouring follows each file section's language, keeps old and new versions apart, survives marker-less and truncated lines, colours header-less review excerpts by their reviewed path, resolves common file types the tokeniser does not name such as `.mjs`, `.h`, `.plist`, and `Gemfile`, renders identically before colouring arrives, and keeps every token colour readable on plain, added, and removed rows in light and dark appearance. |
...
| I10 | `SprintReportPresentationTests.i10ReportMetricsShareCompletedSprintEvidence`, `.chartTabs`, `.durationScale`, `.countAxisLabels` | **Named:** forecast/cumulative usage, cycle duration, accepted outcomes, correction rework, delivery blockers, and adopted improvements are projected from one completed-Sprint datum, with deterministic chart tabs and duration/count scales. |
...
| A08 | P2 | Use the Go menu and keyboard shortcuts for Backlog, Sprint board, Demos, Retrospectives, Reports, Product knowledge, Codebase, Chat, and settings. | P | — |
...
| E09 | P1 | Review one suggested ticket: inspect rationale/dependencies and the "You'll review this as" medium (for a terminal program, "Opens in Terminal"), edit or discuss it, then accept or reject it without changing unrelated proposals. | J | — |
...
### 5.6 Sprint delivery, review, demo, and acceptance — 24
...
| D10 | P0 | Correct up to two sequential structured-result or Demo-contract failures against the preserved workspace before failing; deliver exactly the ticket's owner-approved demo kind (browser, static web prototype, Mac app, terminal app, artifact, command output, or none), pausing with one question when delivery contests it and applying only the owner's answer, never wrapping the product in another surface to comply; carry the prior candidate's validated demo recipe through any revision whose feedback does not name the demo; then recover a failed/interrupted implementation through **Retry work**, reusing the durable workspace/thread contract without duplicating a completed transition. | J | — |
...
| D14 | P0 | Launch, reopen, stop, or retry the exact reviewed demo, including a host-served static web prototype or a program opened in Terminal; a host preparation failure retries preparation only, while a candidate failure requires a new candidate. | J | Y |
...
| D24 | P0 | Open a terminal app demo in Terminal from the exact reviewed revision, bring it forward while it runs, stop it, and retry; the program is the reviewed workspace executable, never a host command. | J+P | — |
...
| V05 | P1 | List independently verified imported and accepted runnable demos — browser, static web prototype, macOS app, and terminal app versions — newest first; omit artifacts and command-output results. | J+P | — |
| V06 | P0 | Open, revisit, switch, stop, and retry exact demos, including a terminal app that reactivates its Terminal window while running and stops through its program; opening another version stops the active one and reconstructs the selected revision. | J | Y |
