...
struct DemoGuidanceTests {
  ...
  func techLeadGuidance() {
    ...
    #expect(instructions.contains("most representative owner-facing result"))
    #expect(instructions.contains("product surface takes precedence"))
    #expect(instructions.contains("code editor, developer tool, or manual setup"))
    #expect(instructions.contains("different states may be covered within the one primary demo"))
    #expect(instructions.contains("Managed preparationCommands and launchCommand are expected"))
    #expect(instructions.contains("verified Environments"))
