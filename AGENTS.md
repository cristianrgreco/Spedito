...
| Epics and suggestions | `PlanningConversationWorkflowCoordinator.swift` owns ordinary epic conversation; `AppModel.swift` composes it and still owns planning with `EpicPlanningRuntime` and `TicketSuggestionRuntime` in `FeatureRuntimeOwners.swift` | `SQLiteStore+Epics.swift`, `SQLiteStore+TicketSuggestions.swift`, `SQLiteStore+Conversations.swift` | `BacklogView.swift`, `EpicDetailView.swift` | E01–E15 |
